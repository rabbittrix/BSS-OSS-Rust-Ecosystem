# 💚 Support the Project

The BSS/OSS Rust ecosystem is community-driven and maintained by professionals who believe in open interoperability.

## Licensing Model

This project uses a dual licensing model:

- **TMF APIs:** Apache 2.0 (commercial use requires authorization and donation)
- **Commercial Components:** Proprietary license required (contact for pricing)

See [LICENSING.md](LICENSING.md) for complete licensing information.

## 💸 How to Contribute Financially

Donations help fund:

- API compliance tests
- TM Forum certification
- Documentation and SDKs
- Infrastructure and hosting
- Community events and outreach

**Donation Link:** [https://opencollective.com/bss-oss-rust](https://opencollective.com/bss-oss-rust)

### Bank Transfer

You can also make a direct bank transfer to support the project. Any amount is greatly appreciated!

**Bank Account Details:**

- **IBAN:** BE72 9670 8136 8616
- **Swift/BIC:** TRWIBEB1XXX
- **Address:** Rue du Trône 100, 3rd floor, Brussels, 1050, Belgium

**Note:** Please include a reference note with your transfer (e.g., "BSS-OSS-Rust donation") so we can properly acknowledge your contribution.

## 💪 Other Ways to Contribute

### Code Contributions

Pull requests are welcome! Please read [CONTRIBUTING.md](CONTRIBUTING.md) for coding standards and review process.

### Documentation

Help improve documentation, write tutorials, or translate content.

### Testing & Feedback

Test the software and provide feedback on issues, usability, and features.

### Community

Help answer questions, participate in discussions, and spread the word.

## 🙏 Thank You

Your support makes this project possible. Every contribution, whether financial or through code, documentation, or feedback, helps build a better BSS/OSS ecosystem for everyone.
